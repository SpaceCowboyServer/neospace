set-game-preset-command-description = Встановлює пресет гри для поточного раунду.
set-game-preset-command-help-text = setgamepreset <id> [кількість раундів, за замовчуванням 1]
set-game-preset-command-hint-1 = <id>
set-game-preset-command-hint-2 = [number of rounds]
set-game-preset-command-hint-3 = [decoy preset]

set-game-preset-optional-argument-not-integer = Якщо надано 2-й аргумент, він має бути числом.
set-game-preset-preset-error = Не вдалося знайти пресет гри "{$preset}"
set-game-preset-decoy-error = If argument 3 is provided it must be a valid preset. Unable to find game preset "{$preset}"

#set-game-preset-preset-set = Set game preset to "{$preset}"
set-game-preset-preset-set-finite = Встановлено ігровий пресет "{$preset}" на наступні {$rounds} раундів.set-game-preset-preset-set-finite-with-decoy = Set game preset to "{$preset}" for the next {$rounds} rounds, showing {$decoy} in the lobby.
